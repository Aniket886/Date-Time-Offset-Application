...
def time_offsets(request):
    ...
    # Compute offsets (default stays 4 hours to preserve existing behavior)
    four_hours_ahead = current_time + datetime.timedelta(hours=offset_hours)
    four_hours_before = current_time - datetime.timedelta(hours=offset_hours)
...
def time_api(request):
    ...
    iso = data.get("iso8601")
    tz = data.get("timezone") or timezone
    return JsonResponse(
        {
            "timezone": tz,
            "iso8601": iso,
